...
#----------------------
step(lm(aveOralM ~ . - SubjectID, data = ici_group2_long), direction = "both")

##or using library(MASS)

stepAIC(lm(aveOralM~. - SubjectID,data=ici_group2_long),direction="both") 


#----------------------
#Forward selection      # CODE RAN: DONE
#----------------------
mint <- lm(aveOralM~1,data=ici_group2_long)
forwardAIC <- step(mint,scope=list(lower=~1, 
                   ...
                   direction="forward", data=ici_group2_long)
...
step(lm(aveOralM~Humidity+T_atm+Distance+Cosmetics+T_FH_Max+T_FHC_Max+canthiMax+T_Max,data=ici_group2_long),direction="backward")
##Best Subset
...
#or
Model=regsubsets(aveOralM~Humidity+T_atm+Distance+Cosmetics+T_FH_Max+T_FHC_Max+canthiMax+T_Max,data=ici_group2_long, nvmax = 10)
summary(Model)
SUM=summary(Model)
names(SUM)
Rsq=SUM$rsq
...
RSS=SUM$rss
#Calculation of AIC
n <- length(ici_group2_long$aveOralM)
